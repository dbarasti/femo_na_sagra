...
const config = {
  ...
  defaultBurgers: [
    {
      name: "Il Montanaro",
      ingredients: ["hamburger", "provola", "speck", "crema_funghi"],
      price: "9.5",
    },
    {
      name: "Classico",
    ...
    {
      name: "BBQ Burger",
      ingredients: ["hamburger", "cheddar", "bacon", "cipolla", "bbq"],
      price: "8.5",
    },
    {
      name: "Sgionfo",
    ...
    {
      name: "Plant-based",
      ingredients: ["vegetariano", "anelli", "zucchine", "pomodoro", "ketchup"],
      price: "8.5",
    },
  ],
  beverages: [
  ...
  config.ingredients.forEach((ingredientType) => {
    ...
    ingredientType.list.forEach((ingredient) => {
      if (document.getElementById(ingredient.id).checked) {
        if (
          ingredientType.type === "Principale" &&
...
function updateOrderTotal() {
  getTotaleDivElement().innerHTML =
    (
      parseFloat(getExtrasDivElement().innerHTML) +
      parseFloat(getBevandeDivElement().innerHTML) +
      parseFloat(getPaninoDivElement().innerHTML)
    ).toString() + " €";
}
...
function clearDefaultBurgersExcept(burgerName) {
  config.defaultBurgers
    .filter((burger) => burger.name !== burgerName)
    .forEach((burger) => {
...
function isPrimarySelected() {
  return config.ingredients
    .filter((ig) => ig.type == "Principale")[0]
    .list.map((i) => isCheckboxWithIdActive(i.id))
    .reduce((acc, curr) => acc || curr, false);
}

function isSecondarySelected() {
  return config.ingredients
    .filter((ig) => ig.type == "Farcitura")[0]
    .list.map((i) => isCheckboxWithIdActive(i.id))
    .reduce((acc, curr) => acc || curr, false);
}
...
function totalBeverageValue(beverage) {
  ...
  return (
    parseFloat(beverage.price) *
    parseFloat(getBeverageQuantityElement(beverage).value)
  );
}
...
function totalExtraValue(extra) {
  ...
  return (
    parseFloat(extra.price) * parseFloat(getExtraQuantityElement(extra).value)
  );
}
